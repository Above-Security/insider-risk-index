...
interface AboveLogoProps extends React.SVGProps<SVGSVGElement> {
  size?: "sm" | "md" | "lg";
...
export function AboveLogoWithText({ 
  size = "md", 
  className,
  textClassName,
  ...props 
}: AboveLogoProps & { textClassName?: string }) {
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <AboveLogo size={size} {...props} />
      <span className={cn("font-medium", textClassName)}>Above Security</span>
    </div>
  );
}